...
class SMTPSession:
    ...
    def _ensure_connected(self) -> None:
        """Reconnect if the connection was dropped (e.g. idle timeout while
        the email body was being generated by OpenAI)."""
        if self._smtp is not None and getattr(self._smtp, "sock", None) is not None:
            try:
                # Cheap liveness probe; if the connection is dead this will
                # raise SMTPServerDisconnected and we'll reconnect below.
                self._smtp.noop()
                return
            except Exception:
                pass
